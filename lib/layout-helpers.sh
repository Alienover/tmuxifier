...
new_window() {
  ...
  if [ -n "$1" ]; then
    tmux set-option -t "$session:$1" allow-rename off
  fi
...
resize_pane_h() {
  tmux resize-pane -t "$session:$windwo.$2" -y "$1"
}

resize_pane_w() {
  tmux resize-pane -t "$session:$windwo.$2" -x "$1"
}
